...
function fillMissingDates(
  data: CategoriesChartProps["data"],
  startDate: Date,
  endDate: Date
): CategoriesChartProps["data"] {
  const result: CategoriesChartProps["data"] = []
  const map = new Map(data.map((item) => [item.date, item]))
  const current = new Date(startDate)

  while (current <= endDate) {
    const dateStr = current.toISOString().split("T")[0]
    result.push(map.get(dateStr) ?? { date: dateStr, categories: 0 })
      ...
      <CardHeader>
        ...
        <CardDescription>
          <span className="hidden @[540px]/card:block">
            الفئات التي تم إنشاؤها مع مرور الوقت
          </span>
          <span className="@[540px]/card:hidden">الجدول الزمني للفئات</span>
        </CardDescription>
