...
export function EventsMetrics({ metrics }: EventMetricsProps) {
  const growthIsPositive = metrics.monthlyGrowth >= 0
  const completionRateGood = metrics.completionRate >= 70
      ...
      <Card className="@container/card relative overflow-hidden border-0 bg-gradient-to-br from-blue-50 via-white to-blue-50 shadow-lg transition-all duration-300 hover:scale-105 hover:shadow-xl">
        ...
        <CardFooter className="pt-0">
          <div className="flex flex-col gap-1 text-xs">
            ...
            <div className="text-muted-foreground">All scheduled events</div>
          </div>
        </CardFooter>
      </Card>
        ...
        <CardFooter className="pt-0">
          <div className="flex flex-col gap-1 text-xs">
            ...
            <div className="text-muted-foreground">
              Average: {metrics.averageDaysUntilEvent}
            </div>
          </div>
        </CardFooter>
      </Card>
      ...
      <Card className="@container/card relative overflow-hidden border-0 bg-gradient-to-br from-orange-50 via-white to-yellow-50 shadow-lg transition-all duration-300 hover:scale-105 hover:shadow-xl">
        ...
        <CardFooter className="pt-0">
          <div className="flex flex-col gap-1 text-xs">
            ...
            <div className="text-muted-foreground">
              {metrics.thisWeekEvents > 0
                ? `${metrics.thisWeekEvents} events scheduled`
                : "No events this week"}
            </div>
          </div>
        </CardFooter>
      ...
      <Card className="@container/card relative overflow-hidden border-0 bg-gradient-to-br from-purple-50 via-white to-pink-50 shadow-lg transition-all duration-300 hover:scale-105 hover:shadow-xl">
        ...
        <CardHeader className="relative pb-3">
          ...
          <div className="flex items-center gap-2">
            <Badge
              variant="outline"
              className={`flex items-center gap-1 text-xs ${completionRateGood ? "border-green-200 bg-green-50 text-green-700" : "border-red-200 bg-red-50 text-red-700"}`}
            >
              {completionRateGood ? (
              ...
              {completionRateGood ? "Excellent" : "Needs Attention"}
            </Badge>
          </div>
        </CardHeader>
        <CardFooter className="pt-0">
          <div className="flex flex-col gap-1 text-xs">
            ...
            <div className="text-muted-foreground">
              {metrics.pastEvents > 0
                ? `${metrics.pastEvents} events completed`
